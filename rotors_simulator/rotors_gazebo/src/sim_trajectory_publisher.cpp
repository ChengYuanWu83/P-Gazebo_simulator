...
int main(int argc, char** argv) {
  ...
  std::ifstream wp_file(args.at(1));
  bool skip_first = 1;
  if (wp_file.is_open()) {
    double t, x, y, z, yaw;
    ...
    while (std::getline(wp_file, line)) {
      ...
      waypoints.push_back(WaypointWithTime(fields[0], fields[1], fields[2], fields[3], fields[4]));
    }
    wp_file.close();
  ...
  for (size_t i = 0; i < waypoints.size(); ++i) {
    ...
    trajectory_point.time_from_start_ns = wp.waiting_time * kNanoSecondsInSecond;

    ROS_INFO("Start publishing %ld waypoints. %ld", i, trajectory_point.time_from_start_ns);
